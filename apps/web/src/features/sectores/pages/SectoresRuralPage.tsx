  ...
  const sortedRecords = useMemo(() => {
    ...
    return [...filteredRecords].sort((a: any, b: any) => {
      ...
      if (sortConfig.key === "municipalidad") {
        ...
        aVal = a.centroPoblado?.codigo || "";
        bVal = b.centroPoblado?.codigo || "";
      } else if (sortConfig.key === "latitud") {
        aVal = a.rural?.latitud ?? -999;
        bVal = b.rural?.latitud ?? -999;
      } else if (sortConfig.key === "longitud") {
        aVal = a.rural?.longitud ?? -999;
        bVal = b.rural?.longitud ?? -999;
      } else if (sortConfig.key === "poblacion") {
        aVal = a.rural?.poblacion ?? -1;
        bVal = b.rural?.poblacion ?? -1;
      } else {
